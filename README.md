...
- **`/notion-sync`** publishes a one-way, read-only view of the pipeline into a Notion database via the official Notion MCP server (OAuth, no API keys) - one row per ranked job plus every tracked application, with a write-once briefing page per row. The repo files stay the system of record: nothing syncs back, and documents sync as filenames only. Complements `/dashboard`: that is the deep interactive view you run at your desk; this is the glanceable live view from anywhere Notion runs (desktop, web, phone).
...
- **`/dashboard`** starts a local interactive dashboard from `job_search_tracker.csv` and the application archives — stat cards, status/sector/channel/funnel charts (inline SVG, no external dependencies), and a filterable applications table. Replaces the old static `/html-report` output with a live local web app that reads the data on demand (no manual re-run needed after `/apply` or `/outcome`, just click Refresh) and can also trigger `/apply` on a pasted URL or job description straight from the page.
...
│   │   ├── dashboard.md               # /dashboard run the local interactive application tracker dashboard
...
├── dashboard/                          # /dashboard local web app (Bun + TypeScript server, static assets, tests)
